...
export function AdminLayout({ children, title }: { children: ReactNode; title: string }) {
  ...
  const NavLinks = () => (
    <>
      <nav className="flex-1 min-h-0 overflow-y-auto space-y-1 px-3 pb-2 pt-2">
      ...
      <div className="border-t border-sidebar-border p-4">
        {user ? (
          <div className="rounded-xl bg-sidebar-accent/40 p-3">
            <div className="text-xs uppercase tracking-widest text-muted-foreground">{user.role}</div>
  ...
  return (
    <div className="h-screen w-full overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-[240px] flex-col border-r border-border bg-sidebar text-sidebar-foreground transition-transform duration-200 md:hidden ${drawerOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between px-5 py-4">
          <div className="flex items-center gap-3">
            <a href="/home-v4" className="flex items-center justify-center rounded-xl bg-white px-2 py-1 hover:opacity-80 transition-opacity">
              <img src="/logo.png" alt="IDEA" className="h-7 w-auto object-contain" />
            </a>
      ...
      <aside className="fixed inset-y-0 left-0 hidden w-[240px] flex-col border-r border-border bg-sidebar text-sidebar-foreground md:flex">
        <div className="flex items-center gap-3 px-5 py-5">
          <a href="/home-v4" className="flex items-center justify-center rounded-xl bg-white px-2.5 py-1.5 hover:opacity-80 transition-opacity">
            <img src="/logo.png" alt="IDEA" className="h-8 w-auto object-contain" />
          </a>
          <div>
            <div className="text-xs font-bold text-foreground/80">Jonny's World</div>
      ...
      <div className="md:pl-[240px] h-full flex flex-col overflow-hidden">
        <header className="shrink-0 z-30 flex items-center justify-between border-b border-border bg-background/80 px-4 py-3 backdrop-blur-md sm:px-6 sm:py-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="text-xs uppercase tracking-widest text-muted-foreground hidden sm:block">Jonny's World</div>
