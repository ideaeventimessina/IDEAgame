...
// WordBack Jonny proposal-phase server timers (sessionId → timer)
const wbJonnyTimers = new Map<string, ReturnType<typeof setTimeout>>();
...
async function _adultAutoChoice(sessionId: string): Promise<void> {
  ...
  await adultUpdate(sessionId, {
    phase: "challenge", chosenType: choice,
    currentChallenge: { text, category: choice, challengeType: choice === "verita" ? "truth" : "dare", durationSeconds, allowPublicVote: choice === "obbligo" },
    challengeEndsAt,
  });
  scheduleAdultChallengeAutoExpire(sessionId, challengeEndsAt);
  logger.info({ sessionId, choice }, "[ADULT_CHOICE] auto-choice by server timer");
}

function scheduleAdultAutoChoice(sessionId: string, deadlineAt: string): void {
  const ex = adultChoiceTimers.get(sessionId); if (ex) clearTimeout(ex);
  const delay = Math.max(0, new Date(deadlineAt).getTime() - Date.now()) + 300;
  const t = setTimeout(() => void _adultAutoChoice(sessionId), delay);
  adultChoiceTimers.set(sessionId, t);
}

// POST /adult/choose — selected player picks obbligo (dare) or verita (truth)
router.post("/home/sessions/:id/adult/choose", async (req, res): Promise<void> => {
  const id = String(req.params["id"]);
  if (!isUUID(id)) { res.status(400).json({ error: "id non valido" }); return; }
  const session = await getSession(id);
  if (!session) { res.status(404).json({ error: "Non trovata" }); return; }
  ...
  req.log.info({ sessionId: id, choice }, "[ADULT_CHOOSE] player chose → challenge");
  res.json({ ok: true });
});
...
router.post("/home/sessions/:id/adult/use-power", async (req, res): Promise<void> => {
  ...
  if (power === "reroll") {
    const level = Number(rp["level"] ?? 1);
    const chosenType = String(rp["chosenType"] ?? "");
    if (chosenType === "verita") {
      const text = pickRandomTruth(level);
      patch["currentChallenge"] = { text, category: "verita", challengeType: "truth", durationSeconds: 60, allowPublicVote: false };
      patch["challengeEndsAt"] = new Date(Date.now() + 60_000).toISOString();
    } else if (chosenType === "obbligo") {
      const text = pickRandomDare(level);
      patch["currentChallenge"] = { text, category: "obbligo", challengeType: "dare", durationSeconds: 90, allowPublicVote: true };
      patch["challengeEndsAt"] = new Date(Date.now() + 90_000).toISOString();
    } else {
      const nc = await pickBottleChallenge(level, (rp["usedChallengeIds"] ?? []) as string[]);
...
router.post("/home/sessions/:id/select-game", async (req, res): Promise<void> => {
  ...
  // ── BYPASS: parola-alle-spalle → WordBack setup menu (preset | Jonny AI) ────────
  if (gameSlug === "parola-alle-spalle") {
    req.log.info({ sessionId: id }, "[FLOW_BYPASS] parola-alle-spalle → home-wordback-setup choice screen");
    const dbSets = await db.select().from(wordBackSetsTable)
      .where(eq(wordBackSetsTable.isActive, true))
      .orderBy(desc(wordBackSetsTable.createdAt)).limit(6);
    const wbSetupPayload: RoundPayload = {
      mode: "home-wordback-setup",
      phase: "setup_choice",
      availableDbThemes: dbSets.map(s => ({ id: s.id, name: s.title, description: s.description ?? "" })),
      builtinPacks: WB_PRESET_PACKS.map(p => ({ id: p.id, name: p.name, emoji: p.emoji })),
      proposals: [],
      proposalEndsAt: null,
      selectedPresetId: null,
      selectedJonnyTheme: null,
      difficulty: null,
      wordCount: null,
    } as unknown as RoundPayload;
    const wbCfg = { ...cfg, phase: "playing", gamesPlayed };
    const [wbUpdated] = await db.update(homeSessionsTable).set({
      gameSlug,
      gameConfig: wbCfg,
      status: "playing",
      currentRound: 0,
      totalRounds: 0,
      roundPayload: wbSetupPayload,
      expiresAt: new Date(Date.now() + 6 * 60 * 60 * 1000),
    }).where(eq(homeSessionsTable.id, id)).returning();
    if (!wbUpdated) { res.status(500).json({ error: "Errore avvio wordback setup" }); return; }
    const wbPlayers = await getPlayers(id);
    emitToRoom(homeRoom(id), "home:game_started", { session: wbUpdated, players: wbPlayers, payload: wbSetupPayload });
    emitHomeState(id, wbUpdated, wbPlayers);
    res.json({ session: wbUpdated, players: wbPlayers });
    return;
  }
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          // __preseed__: rounds already generated by WordBack setup flow (preset or Jonny AI)
          const _sessionCfg = (session.gameConfig ?? {}) as Record<string, unknown>;
          const preloadedRounds = selectedTheme?.id === "__preseed__" && Array.isArray(_sessionCfg["preloadedRounds"]) && (_sessionCfg["preloadedRounds"] as RoundPayload[]).length > 0
            ? (_sessionCfg["preloadedRounds"] as RoundPayload[])
            : await loadGameRoundsForTheme(launchSlug, selectedTheme, selectedSubtype);
...
// ── POST /home/sessions/:id/wordback/select-mode ─────────────────────────────────
// Host picks "preset" (static packs) or "jonny" (AI generation).
router.post("/home/sessions/:id/wordback/select-mode", async (req, res): Promise<void> => {
  const id = String(req.params["id"]);
  if (!isUUID(id)) { res.status(400).json({ error: "id non valido" }); return; }
  const session = await getSession(id);
  if (!session) { res.status(404).json({ error: "Non trovata" }); return; }
  const rp = (session.roundPayload ?? {}) as Record<string, unknown>;
  if (rp["mode"] !== "home-wordback-setup") { res.status(409).json({ error: "Sessione non in modalità wordback-setup" }); return; }
  const { mode: setupMode } = req.body as { mode?: "preset" | "jonny" };
  if (!setupMode || !["preset", "jonny"].includes(setupMode)) {
    res.status(400).json({ error: "mode deve essere 'preset' o 'jonny'" }); return;
  }

  if (setupMode === "preset") {
    const updated = { ...rp, phase: "setup_preset" };
    const [s] = await db.update(homeSessionsTable).set({ roundPayload: updated as unknown as RoundPayload }).where(eq(homeSessionsTable.id, id)).returning();
    const players = await getPlayers(id);
    emitHomeState(id, s, players);
    res.json({ session: s, players });
    return;
  }

  // Jonny mode: open 20-second proposal window
  const proposalEndsAt = new Date(Date.now() + 20_000).toISOString();
  const updated = { ...rp, phase: "setup_jonny_input", proposalEndsAt, proposals: [] };
  const [s] = await db.update(homeSessionsTable).set({ roundPayload: updated as unknown as RoundPayload }).where(eq(homeSessionsTable.id, id)).returning();
  const players = await getPlayers(id);
  emitHomeState(id, s, players);
  res.json({ session: s, players });

  // Auto-advance to theme selection after 20s
  const existing = wbJonnyTimers.get(id);
  if (existing) clearTimeout(existing);
  const timer = setTimeout(async () => {
    wbJonnyTimers.delete(id);
    try {
      const fresh = await getSession(id);
      if (!fresh) return;
      const freshRp = (fresh.roundPayload ?? {}) as Record<string, unknown>;
      if (freshRp["mode"] !== "home-wordback-setup" || freshRp["phase"] !== "setup_jonny_input") return;
      const advanced = { ...freshRp, phase: "setup_jonny_diff" };
      const [adv] = await db.update(homeSessionsTable).set({ roundPayload: advanced as unknown as RoundPayload }).where(eq(homeSessionsTable.id, id)).returning();
      const advPlayers = await getPlayers(id);
      emitHomeState(id, adv, advPlayers);
    } catch (err) { logger.error({ err, sessionId: id }, "[WordBack] jonny proposal timer error"); }
  }, 20_000);
  wbJonnyTimers.set(id, timer);
});

// ── POST /home/sessions/:id/wordback/jonny-propose ───────────────────────────────
// Players propose a theme during setup_jonny_input phase (20-second window).
router.post("/home/sessions/:id/wordback/jonny-propose", async (req, res): Promise<void> => {
  const id = String(req.params["id"]);
  if (!isUUID(id)) { res.status(400).json({ error: "id non valido" }); return; }
  const session = await getSession(id);
  if (!session) { res.status(404).json({ error: "Non trovata" }); return; }
  const rp = (session.roundPayload ?? {}) as Record<string, unknown>;
  if (rp["mode"] !== "home-wordback-setup" || rp["phase"] !== "setup_jonny_input") {
    res.status(409).json({ error: "Proposta non aperta" }); return;
  }
  const { theme, playerId, nickname } = req.body as { theme?: string; playerId?: string; nickname?: string };
  if (!theme || !theme.trim()) { res.status(400).json({ error: "theme obbligatorio" }); return; }
  const trimmed = theme.trim().slice(0, 40);
  const existing = ((rp["proposals"] as Array<{ theme: string }>) ?? []);
  // Max 10 proposals, deduplicate by lowercase
  if (existing.length >= 10 || existing.some(p => p.theme.toLowerCase() === trimmed.toLowerCase())) {
    res.json({ ok: true }); return;
  }
  const proposals = [...existing, { theme: trimmed, playerId: playerId ?? "host", nickname: nickname ?? "?" }];
  const updated = { ...rp, proposals };
  const [s] = await db.update(homeSessionsTable).set({ roundPayload: updated as unknown as RoundPayload }).where(eq(homeSessionsTable.id, id)).returning();
  const players = await getPlayers(id);
  emitHomeState(id, s, players);
...
// ── POST /home/sessions/:id/wordback/jonny-advance ──────────────────────────────
// Host manually closes proposal window and moves to theme+difficulty selection.
router.post("/home/sessions/:id/wordback/jonny-advance", async (req, res): Promise<void> => {
  const id = String(req.params["id"]);
  if (!isUUID(id)) { res.status(400).json({ error: "id non valido" }); return; }
  const session = await getSession(id);
  if (!session) { res.status(404).json({ error: "Non trovata" }); return; }
  const rp = (session.roundPayload ?? {}) as Record<string, unknown>;
  if (rp["mode"] !== "home-wordback-setup") { res.status(409).json({ error: "Sessione non in setup" }); return; }
  // Cancel server timer
  const existing = wbJonnyTimers.get(id);
  if (existing) { clearTimeout(existing); wbJonnyTimers.delete(id); }
  const updated = { ...rp, phase: "setup_jonny_diff" };
  const [s] = await db.update(homeSessionsTable).set({ roundPayload: updated as unknown as RoundPayload }).where(eq(homeSessionsTable.id, id)).returning();
  const players = await getPlayers(id);
  emitHomeState(id, s, players);
  res.json({ session: s, players });
...
// ── POST /home/sessions/:id/wordback/preset-start ────────────────────────────────
// Host picks a preset theme (DB set or built-in pack) + word count → transitions to booking.
router.post("/home/sessions/:id/wordback/preset-start", async (req, res): Promise<void> => {
  const id = String(req.params["id"]);
  if (!isUUID(id)) { res.status(400).json({ error: "id non valido" }); return; }
  const session = await getSession(id);
  if (!session) { res.status(404).json({ error: "Non trovata" }); return; }
  ...
  const { themeId, themeName, count } = req.body as { themeId?: string; themeName?: string; count?: number };
  if (!themeId || !themeName) { res.status(400).json({ error: "themeId e themeName obbligatori" }); return; }
  const wordCount = Math.min(Math.max(Number(count ?? 10), 5), 20);

  // Load rounds: DB set (UUID) or built-in pack (string id like "film")
  let rounds: RoundPayload[];
  if (isUUID(themeId)) {
    // Load from DB card set
    const cards = await db.select().from(wordBackCardsTable)
      .where(and(eq(wordBackCardsTable.setId, themeId), eq(wordBackCardsTable.isActive, true)))
      .orderBy(asc(wordBackCardsTable.orderIndex));
    if (cards.length === 0) {
      rounds = generateWordBackFallback("cultura", wordCount).map(r => r as unknown as RoundPayload);
    } else {
      const allWords = cards.map(c => c.word);
      rounds = shuffleArr(cards).slice(0, wordCount).map((c, i) => ({
        mode: "home-wordback", roundIndex: i, setName: themeName,
        word: c.word, tabooWords: generateTabooWords(c.word, c.hint ?? "", allWords, c.category ?? ""),
        hint: c.hint ?? "", category: c.category ?? "", difficulty: c.difficulty ?? "medium",
        points: c.points ?? 150, timeLimit: c.timeLimit ?? 45, guessed: false,
      } as unknown as RoundPayload));
    }
  } else {
    // Built-in preset pack
    rounds = generateWordBackFallback(themeId, wordCount).map(r => r as unknown as RoundPayload);
  }

  const cfg = (session.gameConfig ?? {}) as Record<string, unknown>;
  const newCfg = { ...cfg, preloadedRounds: rounds };
  const bookingRp: RoundPayload = {
    mode: "home-flow",
    gameFlowPhase: "booking",
    gameSlug: "parola-alle-spalle",
    selectedTheme: { id: "__preseed__", name: themeName },
    bookedPlayers: [],
    maxPlayers: 2,
  } as unknown as RoundPayload;

  const [s] = await db.update(homeSessionsTable)
    .set({ roundPayload: bookingRp, gameConfig: newCfg, totalRounds: rounds.length })
    .where(eq(homeSessionsTable.id, id)).returning();
  if (!s) { res.status(500).json({ error: "Errore avvio preset" }); return; }
  const players = await getPlayers(id);
  emitToRoom(homeRoom(id), "home:round", { round: 0, payload: bookingRp });
  emitHomeState(id, s, players);
  res.json({ session: s, players });
});

// ── POST /home/sessions/:id/wordback/jonny-generate ─────────────────────────────
// Host confirms theme + difficulty + count → AI generates rounds → booking.
router.post("/home/sessions/:id/wordback/jonny-generate", async (req, res): Promise<void> => {
  const id = String(req.params["id"]);
  if (!isUUID(id)) { res.status(400).json({ error: "id non valido" }); return; }
  const session = await getSession(id);
  if (!session) { res.status(404).json({ error: "Non trovata" }); return; }
  ...
  const { theme, difficulty, count, fallbackThemeId } = req.body as {
    theme?: string; difficulty?: "easy" | "medium" | "hard"; count?: number; fallbackThemeId?: string;
  };
  if (!theme || !theme.trim()) { res.status(400).json({ error: "theme obbligatorio" }); return; }
  const wordCount = Math.min(Math.max(Number(count ?? 10), 5), 20);
  const diff = (["easy", "medium", "hard"].includes(difficulty ?? "") ? difficulty : "medium") as "easy" | "medium" | "hard";

  // Set generating phase immediately so TV shows loading
  const generatingRp = { ...rp, phase: "setup_jonny_generating", selectedJonnyTheme: theme.trim(), difficulty: diff, wordCount };
  await db.update(homeSessionsTable).set({ roundPayload: generatingRp as unknown as RoundPayload }).where(eq(homeSessionsTable.id, id));
  const genPlayers = await getPlayers(id);
  const genSession = await getSession(id);
  if (genSession) emitHomeState(id, genSession, genPlayers);
  res.json({ ok: true, generating: true });

  // Generate in background
  void (async () => {
    try {
      const rounds = await generateWordBackRounds(theme.trim(), wordCount, diff, fallbackThemeId ?? "cultura");
      const roundPayloads = rounds.map(r => r as unknown as RoundPayload);
      const cfg = (session.gameConfig ?? {}) as Record<string, unknown>;
      const newCfg = { ...cfg, preloadedRounds: roundPayloads };
      const bookingRp: RoundPayload = {
        mode: "home-flow",
        gameFlowPhase: "booking",
        gameSlug: "parola-alle-spalle",
        selectedTheme: { id: "__preseed__", name: theme.trim() },
        bookedPlayers: [],
        maxPlayers: 2,
      } as unknown as RoundPayload;
      const [s] = await db.update(homeSessionsTable)
        .set({ roundPayload: bookingRp, gameConfig: newCfg, totalRounds: roundPayloads.length })
        .where(eq(homeSessionsTable.id, id)).returning();
      if (s) {
        const players2 = await getPlayers(id);
        emitToRoom(homeRoom(id), "home:round", { round: 0, payload: bookingRp });
        emitHomeState(id, s, players2);
      }
    } catch (err) {
      logger.error({ err, sessionId: id }, "[WordBack] jonny-generate background error");
    }
  })();
});
